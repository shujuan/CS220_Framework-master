...
bool Utilities::GRGrid::isLocalBottleNeck( UINT ax, UINT by, int netID )  {
	...
	// We have to have at least 3 cells in
	// order to have a bottle neck ***********
...
bool Utilities::GRGrid::fastRetraceGRPath( GRNode * source, GRNode *target ) {
...
void Utilities::GRGrid::writeGridDiskNets( int filen ) {
	...
	char buffer1[32] ;
	itoa (filen,buffer1,10);

	char buffer2[256] ;
	strcpy(buffer2, "c:/testcases/file0" );
		...
		for ( int y=0; y < this->get_height(); y++ )  {
			...
			for ( int x=0; x < this->get_width(); x++ )  {
				...
				if ( gnode->getPin() ) {
					...
					file << " P " << gnode->getX() << " " << gnode->getY() << " " << gnode->getNetID() << " | " ;
					continue ;
				}

				file << " C " << gnode->getX() << " " << gnode->getY() << " " << gnode->getNetID() << " | " ;
...
void Utilities::GRGrid::writeGridDiskLee( int filen ) {
	...
	strcpy(buffer2, "c:/testcases/lfile0" );
		...
		for ( int y=0; y < this->get_height(); y++ )  {
			...
			for ( int x=0; x < this->get_width(); x++ )  {
				...
				if ( gnode->getPin() ) {
					...
					file << " P " << gnode->getX() << " " << gnode->getY() << " " << gnode->getLeeMark() << " | " ;
					continue ;
				}

				file << " C " << gnode->getX() << " " << gnode->getY() << " " << gnode->getLeeMark() << " | " ;
...
void Utilities::GRGrid::writeGridDiskPri( int filen ) {

	char buffer1[32] ;
	itoa (filen,buffer1,10);

	char buffer2[256] ;
	strcpy(buffer2, "c:/testcases/pfile0" );
		...
		for ( int y=0; y < this->get_height(); y++ )  {
			...
			for ( int x=0; x < this->get_width(); x++ )  {
				...
				if ( gnode->getInBlock() ) {
					...
					file << " B " << gnode->getX() << " " << gnode->getY() << " 0" << " | " ;
					continue ;
				...
				if ( gnode->getPin() ) {
					...
					file << " P " << gnode->getX() << " " << gnode->getY() << " " << gnode->getPriority() << " | " ;
					continue ;
				}

				file << " C " << gnode->getX() << " " << gnode->getY() << " " << gnode->getPriority() << " | " ;
